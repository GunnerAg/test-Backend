...
//--------- Parseo los datos de ventas.csv para obtener categoria, coste y cantidades de cada venta y paraslos de argumento a la funcion calculatePrice------------//
    ...
    let parser = parse({columns: true, delimiter: ';'}, function (err,data) {
    data.map((venta)=>{
    ...
    fs.createReadStream(`${__dirname}/dataset/${process.argv[2]}.csv`).pipe(parser)
    ...
    let datosraw = fs.readFileSync(`./dataset/${process.argv[3]}.json`);
